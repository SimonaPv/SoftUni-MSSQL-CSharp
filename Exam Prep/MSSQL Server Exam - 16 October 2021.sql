...
CREATE TABLE Cigars(
	...
	BrandId INT NOT NULL FOREIGN KEY REFERENCES Brands,
	TastId INT NOT NULL FOREIGN KEY REFERENCES Tastes,
	SizeId INT NOT NULL FOREIGN KEY REFERENCES Sizes,
...
CigarId INT NOT NULL FOREIGN KEY REFERENCES Cigars,
...
JOIN Sizes AS s
	ON c.SizeId = s.Id
WHERE s.[Length] >= 12 AND (CigarName LIKE '%ci%' OR c.PriceForSingleCigar > 50) AND s.RingRange > 2.55
...
AS
BEGIN
	DECLARE @id INT = (SELECT Id FROM Clients WHERE FirstName = @firstName)
	RETURN (SELECT 
				COUNT(*)
			FROM Clients AS c
			JOIN ClientsCigars AS cc
